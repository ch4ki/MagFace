...
def process_faces(path: str, save_path : str):
    ...
    # magFace.to(device)
    dirs = [Path("img_list")] #[os.path.join(path, dir) for dir in os.listdir(path) if os.path.isdir(os.path.join(path, dir))]
    ...
    for directory in dirs:
        ...
        inf_loader = torch.utils.data.DataLoader(inf_dataset, batch_size=1, num_workers=1, shuffle=False)
        batch_time = utils.AverageMeter('Time', ':6.3f')
        data_time = utils.AverageMeter('Data', ':6.3f')
        progress = utils.ProgressMeter(len(inf_loader), [batch_time, data_time], prefix="Extract Features: ")
