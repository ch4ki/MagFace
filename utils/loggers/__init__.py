...
class Loggers():
    def __init__(self, save_dir=None, weights=None, opt=None, hyp=None, logger=None, include=LOGGERS):
        ...
        if not wandb:
            ...
            s = f"{prefix}run 'pip install wandb' to automatically track and visualize model 🚀 runs (RECOMMENDED)"
        ...
        if wandb and 'wandb' in self.include:
            ...
            if pkg.parse_version(wandb.__version__) >= pkg.parse_version('0.12.11'):
                self.logger.warning(
                    "Project temporarily requires wandb version 0.12.10 or below. Some features may not work as expected."
                )
        else:
            self.wandb = None
